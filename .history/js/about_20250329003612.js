...
const correctAnswers = ['pink', 'fall', 'bunny', 'tangled'];
const dressImages = [
...
function checkAnswer(questionNumber, selectedAnswer) {
    ...
    if (selectedAnswer === correctAnswers[questionNumber - 1]) {
        ...
        // Update the dress image for the next question 
        currentDressImage = dressImages[questionNumber];  
...
function finishGame() {
    ...
    if (correctAnswersGiven === 4) {
        document.querySelector("#end-message").textContent = "Congratulations! You got all answers correct! Ms. Bunny is off to the royal ball 🩵";
        document.querySelector("#end-image").src = "../img/about/bunny-prince.png"; // Success Image
    } else {
        document.querySelector("#end-message").textContent = "Oh, maybe Ms. Bunny needs more time to dress.";
        document.querySelector("#end-image").src = "../img/about/bunny-sitting.png"; // Failure Image
    }
...
function resetGamePosition() {
    ...
    gameLandingContent.style.top = "50%";  
    gameLandingContent.style.left = "50%"; 
    gameLandingContent.style.transform = "translate(-50%, -50%)"; 
}
